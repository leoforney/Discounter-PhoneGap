...
function usage
{
    echo -e "S3 Travis v$VERSION"
    echo -e "iLeoable - leo@crystalnetwork.us\n"
    ...
    if grep -q "HTTP/1.1 400" "$RESPONSE_FILE"; then
        ...
        case $ERROR_MSG in
             *access?attempt?failed?because?this?app?is?not?configured?to?have*)
                echo -e "\nError: The Permission type/Access level configured doesn't match the S3 App settings!\nPlease run \"$0 unlink\" and try again."
                exit 1
            ;;
        esac
...
function db_stat
{
    ...
    #Even if the file/dir has been deleted from S3 we receive a 200 OK response
...
#Returns the free space on S3 in bytes
...
function db_unlink
{
    echo -ne "Are you sure you want unlink this script from your S3 account? [y/n]"
    read answer
    if [[ $answer == "y" ]]; then
        rm -fr "$CONFIG_FILE"
        echo -ne "DONE\n"
...
if [[ -e $CONFIG_FILE ]]; then
    ...
    #Back compatibility with previous S3 Uploader versions
...
#NEW SETUP...
    ...
    echo -ne " 2) Click on \"Create App\", then select \"S3 API app\"\n"
    ...
    while (true); do
        ...
        echo -n " # Permission type, App folder or Full S3 [a/f]: "
        read ACCESS_LEVEL
    ...
    while (true); do
        ...
        #USER AUTH
        echo -ne "\n Please open the following URL in your browser, and allow S3 Uploader\n"
        echo -ne " to access your S3 folder:\n\n --> ${API_USER_AUTH_URL}?oauth_token=$OAUTH_TOKEN\n"
